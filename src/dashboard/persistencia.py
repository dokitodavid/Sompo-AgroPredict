...
1) SQLite LOCAL (`data/historico_local.db`) — uma tabela que cresce
   ...
   que hoje o projeto ainda nao tem, conforme documentado no README.
...
def registrar_acesso(usuario: str, evento: str, sucesso: bool) -> int:
    """Registra login, logout e tentativas negadas sem armazenar senhas."""
    conn = _conectar_local()
    cur = conn.execute(
        "INSERT INTO log_acessos (data_hora, usuario, evento, sucesso) VALUES (?,?,?,?)",
        (
            datetime.now().isoformat(timespec="seconds"),
            (usuario or "nao_informado")[:120],
            evento[:60],
            int(sucesso),
        ),
    )
...
def registrar_oracle(config_oracle: dict, registro: dict,
    ...
    """
    if not config_oracle or not all(config_oracle.get(k) for k in ("user", "password", "dsn")):
        return False
