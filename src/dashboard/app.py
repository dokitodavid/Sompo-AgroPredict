...
def _config_secreta(nome: str) -> str | None:
    """Le configuracao sensivel do ambiente ou de .streamlit/secrets.toml."""
...
def exigir_autenticacao() -> None:
    """Bloqueia o dashboard ate um login valido e registra as tentativas."""
    usuario_config = _config_secreta("APP_USERNAME")
    senha_config = _config_secreta("APP_PASSWORD")

    if not usuario_config or not senha_config:
        st.error(
            "Autenticação não configurada. Defina APP_USERNAME e APP_PASSWORD "
    ...
    if st.session_state.get("_autenticado"):
        with st.sidebar:
            st.caption(f"Usuário: {st.session_state['_usuario_autenticado']}")
            if st.button("Sair", key="logout"):
                registrar_acesso(st.session_state["_usuario_autenticado"], "logout", True)
                st.session_state.pop("_autenticado", None)
                st.session_state.pop("_usuario_autenticado", None)
                st.rerun()
        return

    st.title("🔐 Acesso ao Sompo AgroPredict")
    ...
    if enviar:
        valido = hmac.compare_digest(usuario, usuario_config) and hmac.compare_digest(senha, senha_config)
        registrar_acesso(usuario, "login", valido)
        if valido:
            st.session_state["_autenticado"] = True
            st.session_state["_usuario_autenticado"] = usuario
            st.rerun()
...
def carregar_modelo():
    ...
    # O dashboard processa uma leitura por vez; uma unica thread evita criar
    # pools desnecessarios e melhora a compatibilidade em ambientes restritos.
    if "clf" in modelo_carregado.named_steps:
        modelo_carregado.named_steps["clf"].set_params(n_jobs=1)
    return modelo_carregado


@st.cache_data
def buscar_historico_oracle():
    if not all(CONFIG_ORACLE.values()):
        return None
    try:
        import oracledb
...
score        = calcular_score_risco(classes_modelo, probas, override_alto=em_max)
recomendacao = gerar_recomendacao(entrada_dict, classe_pred, vars_max)

# =====================================================================
# Persistência — SQLite principal + Oracle opcional
# =====================================================================
# Em telemetria, o throttle evita gravar a cada rerun de componente. No modo
# manual, uma ação explícita garante que a avaliação demonstrada seja salva.
INTERVALO_MIN_GRAVACAO = 10  # segundos — mesmo ritmo do ciclo de telemetria
agora_ts = time.time()
pode_gravar_automaticamente = (
    modo_real
    and (agora_ts - st.session_state.get("_ultimo_save", 0) >= INTERVALO_MIN_GRAVACAO)
)
salvar_manualmente = (
    st.sidebar.button("💾 Salvar avaliação no SQLite", type="primary", width="stretch")
    if not modo_real else False
)
...
registro = {
    **entrada_dict,
    ...
    "fonte_velocidade": (
        "manual" if manual_velocidade or velocidade_gps is None else origem_velocidade
    ),
    "fonte_clima": (
        "manual" if manual_clima or not (clima and clima.get("ok")) else "open-meteo"
    ),
    "fonte_horas": origem_horas,
}

if pode_gravar_automaticamente or salvar_manualmente:
    gravou_oracle = registrar_oracle(CONFIG_ORACLE, registro)
    id_local = registrar_local(registro, gravado_oracle=gravou_oracle)
    st.session_state["_ultimo_save"] = agora_ts
    if salvar_manualmente:
        st.sidebar.success(f"Avaliação #{id_local} salva no SQLite.")
...
# =====================================================================
st.markdown("---")
st.subheader("🔍 Quais variáveis influenciam o modelo? — Feature Importance")
st.markdown(
    "O gráfico mostra a importância global de cada variável no Random Forest. "
    "Ele ajuda a entender o comportamento geral do modelo, mas não explica isoladamente uma predição."
)
...
with tab_local:
    ...
    st.caption(
        "Essa tabela cresce automaticamente a cada 10s na telemetria ou pelo botão de salvar no modo "
        "manual. É gravada localmente e não depende do Oracle."
    )
    if total_local > 0:
        df_local = carregar_local(limite=200)
        st.dataframe(df_local, width="stretch", height=320)
    if not modo_real:
        st.info("Modo manual: use o botão `Salvar avaliação no SQLite` na barra lateral.")
...
with tab_oracle:
    ...
    if historico is not None and len(historico) > 0:
        ...
        with col_h2:
            st.dataframe(historico.head(20), width="stretch")
    else:
        st.info(
...
st.markdown("---")
st.caption(
    f"Sompo AgroPredict · Sprint 3 · Challenge Sompo × FIAP · "
